...
| Platform pack | [`platform/`](platform/) | for the shape where the app team holds a project and not the cluster: `render.sh` puts every cluster-scoped object into ONE file a platform team applies once, and `install.sh --cluster-scope platform-pack` then does namespace-scoped work only — refusing by name if the pack is not there. Not needed when the install runs with cluster rights, which is the default. |
