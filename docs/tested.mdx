...
[`kit/report/`](upgrade) is the door for a deployment that already exists. It is **fixture-tested end to end, and it has read exactly one real estate** — the live deployment of [the from-docs run](#the-from-docs-run-2026-08-27), which is where the 270-line `state-report.yaml` on the [upgrade](upgrade) page came from.

| Claim | Proven where | Not yet |
|---|---|---|
| The tool itself | 46 tests against a fake `kubectl`, where the fixture directory *is* the estate (`make test-report`), plus one live run — exit 0, one 270-line report — against a throwaway LKE cluster deployed from these docs ([the from-docs run](#the-from-docs-run-2026-08-27)) | Any estate with history: that one live cluster was hours old and built for the purpose. Nothing says what the reporter does against a long-lived deployment that is not ours |
...
| **Entry expiry, revocation, contract clauses** | Live against a scratch channel on the real registry, with the stale-entry message deliberately distinct from the signature message ([channel hardening](receipts/2026-08-25-channel-hardening)) | **Measured 2026-08-25:** the deployed verifier was v0.1.1 and did not check freshness, so nothing enforced expiry inside a cluster. No run since has re-read the deployed version — treat the currency of this row as unverified, not the claim |
