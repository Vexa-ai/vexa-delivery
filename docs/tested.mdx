...
- **The station report.** [`vexa_validate.py`](install#step-5-send-the-station-bundle-back) writes `station-report.yaml` — one commented file carrying your provider profile, your values with every secret-shaped value replaced by `REDACTED`, the contract, and the preflight and smoke receipts, each with the sha256 of its own text. Read it, then send it. That report is exactly what a `PROFILE_TESTED=no` line is waiting for.
