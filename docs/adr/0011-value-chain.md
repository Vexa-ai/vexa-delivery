...
**The committed file is the artifact; regenerating is not the verification path.** A
manifest is byte-reproducible from the same base, candidate and inputs *except for
`generated_at`* — so a second run of the generator produces a different sha256 for the same
candidate. That is not drift and a reader must not read it as drift: the identity is the bytes
of the document that was committed and signed, and it is verified by re-hashing that file, never
by regenerating and comparing. (Verified on t26-01226: two runs, identical in every field but
`generated_at`.)
