...
2. **License:** proprietary short-form "Vexa Enterprise license", GitLab-EE style, holder Vexa
   ...
   deliberately does not encode a pricing unit. Apache-2.0 material copied from `Vexa-ai/vexa` stays Apache-2.0 with attribution;
   under ADR-0008 so does everything else here.
3. **Charter is carried, not restated:** This ADR series (from 0001) owns delivery-system decisions; product-level decisions
