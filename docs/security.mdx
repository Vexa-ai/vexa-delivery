...
Your credential is **pull-only**, scoped to your channel, revocable independently of every other subscriber's, enforced at the edge proxy rather than by the registry ([why](channel#worth-knowing)). It reaches you age-encrypted to a key you already control, never in plaintext, on a different route from the public key it pairs with. **Vexa holds no credential of yours** — there is no inbound access to grant.

[Preflight](preflight) reports, before install, which workloads OpenShift SCC `restricted-v2` or PodSecurity `restricted` would refuse **as written** — static analysis of the delivered manifests, not a live admission test, so a cluster whose mutating webhooks inject a security context can admit a pod the preflight flags. Any read the preflight is refused is reported `UNKNOWN` and named, never guessed at.
