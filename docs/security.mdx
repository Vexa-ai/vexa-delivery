...
**Nothing reaches in: your cluster pulls, verifies with keys you pin, and promotes to production on your attestation.** One component of ours runs in your cluster: the PreSync [verifier](verify), a shell script on Alpine with cosign, oras and jq. It reads the channel and its own policy, writes nothing, and is Apache-2.0 like everything else. Nothing else of ours executes there, nothing of ours reaches outward, and nothing proprietary runs at all.
...
| [The station report](install#step-5-send-the-station-bundle-back) | One file, written to disk and sent by hand: substrate facts, values with secrets replaced, the contract, the receipts. You read all of it first — that is what one file is for |
