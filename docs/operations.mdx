...
```bash
./kit/self-update.sh --check        # cloned layout (step 1's default): what would move
./kit/self-update.sh               # move it

./vexa-kit/self-update.sh --check   # bootstrap layout, where the kit unpacked to ./vexa-kit
```
