...
Every estate that has been running has state an upgrade must survive — rows that predate a constraint, a model pinned to a device, a schema last converged three releases ago. Send us that state and the upgrade arrives on your channel already rehearsed against it: [Upgrade report](upgrade-report). Read-only, one command, nothing transmits.
