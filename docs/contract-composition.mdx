...
has one contract; a subscriber shape such as `pilot-stable` has one; and a station — staging, prod, a
...
the in-cluster verifier does not evaluate the filled contract. Since 2026-08-31 the gate no longer
admits an entry on its carriage block alone: where the carriage sets `require_entry_values_proven`,
the verifier adjudicates `required_values[]` against the entry's own `values_proven` block, one
roster line per value id. That is a narrower thing than reading a filled contract — it checks that a
named station claimed each required value and carried evidence for the claim, not that the claim is
true — but the clause is no longer void.
...
**The `pilot-stable` contract** — a different channel, extending it: raises one, adds one:

```json
{ "contract_id": "pilot-stable-2026-09",
...
**The compiled effective `pilot-stable` contract** — what that channel's charts mount and its verdicts
pin, at every station on it:

```json
{ "contract_id": "pilot-stable-2026-09",
  "compiled_from": ["vexa-internal-2026-09", "pilot-stable-2026-09"],
  "required_values": [
    ...
    { "value_id": "V-join",  "claim": "A bot joins a live meeting and segments arrive over websocket.",
      "rank": "autonomous", "validation": {"kind": "station-smoke"},
      "enforcement": "required", "freshness_days": 3, "from": "pilot-stable-2026-09" },
    { "value_id": "V-api",   "claim": "The documented API surface responds as documented.",
      "rank": "autonomous", "validation": {"kind": "set-test", "subject": "digest-set"},
      "enforcement": "advisory", "freshness_days": 14, "from": "pilot-stable-2026-09" }
  ],
  "carriage": { "require_publication_mode": "candidate", "max_entry_age_days": 30 },
...
Every train that runs on `pilot-stable` fills in that same compiled document — the demand does not
...
produces one: the seq-7 instance is authored **by hand** and the in-cluster verifier does not
evaluate it. What the gate does read, since 2026-08-31, is the entry's own `values_proven` block
against the contract's `required_values[]` — so required values are checked by roster rather than
by reading demand rows against proof rows in one pass, and an entry that answers none of them is
refused instead of admitted on its carriage block alone. See
