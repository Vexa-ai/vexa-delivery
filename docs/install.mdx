...
**Set your transcription endpoint before step 4.** The example values file does not carry one, and a cluster that is Synced and Healthy still cannot dispatch a bot until `transcriptionServiceUrl` and `transcriptionServiceToken` are set in `my-values.yaml` — [what's proven, and where](tested#install-and-adoption).
