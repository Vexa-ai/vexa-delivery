...
**This is the path our own production runs on, and the only one exercised end-to-end: Linode LKE.** `lke` is the single profile carrying `PROFILE_TESTED=yes` — installed from nothing on a throwaway LKE cluster ([receipt](/receipts/2026-08-21-m2-throwaway-test)), and Vexa's own production cluster adopted by its channel on 2026-08-25 with 36 of 36 pods keeping their UID through the sync ([receipt](/receipts/2026-08-25-prod-adoption)). A third run resolved the whole 115-object estate onto an empty cluster from signed artifacts alone; 16 of 24 workloads reached Running and the rest are written up as findings in [that receipt](/receipts/2026-08-25-throwaway-clean-pull) rather than fixed by it. `--provider generic` differs from `lke` only in that flag and the notes beside it: no conformant cluster other than LKE has run this end-to-end, so on anything else expect deltas and let the preflight find them. Per-provider deltas: [GKE](/environments/gke) · [EKS](/environments/eks) · [AKS](/environments/aks) · [OpenShift](/environments/openshift).
