...
Five files plus a manifest. The customer produces them; `kit validate` packs
them into `station-<name>.tar.gz`.

| File | What it carries | Who writes it |
|---|---|---|
| `profile.env` | Substrate facts: provider, k8s version, scope (`cluster`/`namespace`), namespace, storage class, PSA mode, whether a LimitRange/ResourceQuota is present, mirror host. Never credentials. | kit, from the cluster |
| `values.redacted.yaml` | The customer-local values ([`kit/profiles/vexa/customer-values.example.yaml`](../kit/profiles/vexa/customer-values.example.yaml)) with every credential replaced by a placeholder. Only the SHAPE travels. | customer, redacted by kit |
| `contract.yaml` | The station half of [`kit/verify/policy.example.yaml`](../kit/verify/policy.example.yaml) plus a `require:` list — the per-release guarantees this station demands. | the customer |
| `preflight-receipt.txt` | The P1–P9 conformance run from [`kit/preflight/vexa_preflight.py`](../kit/preflight/vexa_preflight.py). | kit, on their cluster |
| `smoke-receipt.json` | The last end-to-end run at the station: dispatch → admit → transcript → teardown. | kit, on their cluster |
| `station.json` | Manifest: schema version, station name, customer, environment, kit version, created-at, and the `sha256` of every file above. | kit |

`station.json` is what makes the bundle a bundle rather than a folder: it
names the station, and its digests are checked against the bytes on ingest, so
a file edited in transit is a refusal, not a surprise later.
...
python3 kit/validate/vexa_validate.py --namespace <ns> \
    --customer-values my-values.yaml --flows --out station.tar.gz
```

It runs preflight, runs the smoke, redacts the values, copies the contract
they maintain, writes `station.json`, and packs the files. Nothing leaves
their cluster except those files, and none of them carries a credential.

> **Rung, honestly:** the bundle format, the publisher side, and
> `kit/validate/vexa_validate.py` as one command are all merged on `main`. What
> is still synthetic is the **worked example**: the rehearsal bundle below was
> hand-assembled from this repo's own example files with **fabricated**
> preflight and smoke receipts, both labelled SYNTHETIC. The first real bundle
> is a subscriber's, and it will not be committed here (see the `stations/`
> rule in `.gitignore`).
...
- **S2 completeness** — all five files plus `station.json`, each matched
  unambiguously to its role.
- **S3 manifest identity** — `station.json` names *this* station, every
  declared digest matches the bytes, and nothing undeclared rides along.
- **S4 no plaintext secrets** — **defense in depth.** The customer redacts;
  we refuse to *hold* what they missed. Two scans: secret-shaped keys carrying
  real values (a key ending in `Name`/`Ref`/`Path` is a reference, not a
  credential), and credential-shaped patterns anywhere in the bytes (PEM
  private keys, `sk-`/`sk-ant-`, `ghp_`, `AKIA…`, `AIza…`, `xox…`, JWTs,
  `client-key-data`). **A refusal never prints the value** — file, line and
  rule only, so the refusal text is safe to paste into an issue.

On pass it unpacks into `stations/<name>/` and writes `ingest-receipt.json`
with the ingested-at stamp, the bundle's own digest, and the file digests.
...
- **S5 render** — `helm template` with the station's `values.redacted.yaml`
  over the chart defaults. Redacted placeholders are fine: the gate reads
  shapes, not credentials. A render failure is a refusal — better here than in
  their Argo.
...
[`rehearsal/`](rehearsal/) — the SYNTHETIC worked example, ingested from a
hand-assembled bundle, with a gate report against the packaged 0.12.26 chart.
It exists so the shape is readable in one sitting.

**Real customer stations are not committed.** `values.redacted.yaml` is a
